...
train_dir = "../../../../storage/amit/COCO/train2017/"
val_dir =  "../../../../storage/amit/COCO/validation2017/"
train_image_path = [train_dir+file for file in os.listdir(train_dir) if file.endswith(".jpg")]
val_image_path = [val_dir+file for file in os.listdir(val_dir) if file.endswith(".jpg")]
...
class DataGenerator(keras.utils.Sequence):
    ...
    def load_image(self,image_path):
        ...
        image_reduced = self.resize_crop(image)
...
def jigsaw(shape=(NUM_PATCHES,PUZZLE_PATCH,PUZZLE_PATCH,CHANNELS)):
    ...
    return jigsaw_model
model_COCO = jigsaw(shape=(NUM_PATCHES,PUZZLE_PATCH,PUZZLE_PATCH,CHANNELS))
### Training Model

#### Tensorboard logger and early stopping
checkpointer= ModelCheckpoint("./models/ensemble/jigsaw_COCO5.h5",verbose=1,save_best_only=True)
file_name="jigsaw_COCO5"
...
model_COCO.compile(optimizer=optimizer,loss=loss,metrics=[train_acc_metric])
#### Train Model
history = model_COCO.fit(train_generator,validation_data=val_generator,epochs=350,callbacks=callbacks)
