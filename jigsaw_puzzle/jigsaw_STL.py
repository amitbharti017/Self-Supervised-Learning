...
import datetime
#### 2. Dataset Path
train_dir = "../../../../storage/amit/STL/train/"
val_dir =  "../../../../storage/amit/STL/validation/"
train_image_path = [train_dir+file for file in os.listdir(train_dir) if file.endswith(".png")]
val_image_path = [val_dir+file for file in os.listdir(val_dir) if file.endswith(".png")]
...
def jigsaw(shape=(NUM_PATCHES,PUZZLE_PATCH,PUZZLE_PATCH,CHANNELS)):
    ...
    return jigsaw_model
model_STL = jigsaw(shape=(NUM_PATCHES,PUZZLE_PATCH,PUZZLE_PATCH,CHANNELS))
### Training Model
#### Tensorboard logger and early stopping
checkpointer= ModelCheckpoint("./models/ensemble/jigsaw_STL5.h5",verbose=1,save_best_only=True)
file_name="jigsaw_STL5"
...
model_STL.compile(optimizer=optimizer,loss=loss,metrics=[train_acc_metric])
#### Train Model
history = model_STL.fit(train_generator,validation_data=val_generator,epochs=350,callbacks=callbacks)