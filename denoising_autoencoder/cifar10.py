...
model_cifar10=tf.keras.Model(inputs=[start],outputs=[outputs])
model_cifar10.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001),loss="mse", metrics=[tf.keras.metrics.MeanSquaredError()])
model_cifar10.summary()
#### Callback functions
checkpointer= tf.keras.callbacks.ModelCheckpoint("./models/ensemble/cifar10_5.h5",verbose=1,save_best_only=True)
file_name="denoise_cifar10_5"
...
#### Train Model
history = model_cifar10.fit(x_train_noisy,x_train,validation_data=(x_test_noisy,x_test),batch_size=32,epochs=100,callbacks=callbacks)
