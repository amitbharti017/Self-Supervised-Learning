...
val_dir = "../../datasets/tiny_imagenet/validation/"
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self,current_batch):
        ...
        for file in current_batch:
            ...
            image=image/255.0 #Normalizing the image pixel values to be between 0 and 1
...
model_tinyimagenet=tf.keras.Model(inputs=[start],outputs=[outputs])
model_tinyimagenet.compile(optimizer=Adam(learning_rate=0.0001),loss="mse", metrics=[tf.keras.metrics.MeanSquaredError()])
model_tinyimagenet.summary()
checkpointer= tf.keras.callbacks.ModelCheckpoint("./models/ensemble/tinyimagenet5.h5",verbose=1,save_best_only=True)

file_name="denoise_tinyimagenet5"
...
val_generator = DataGenerator(val_image_path,**params)
history = model_tinyimagenet.fit(train_generator,validation_data=val_generator,epochs=100,callbacks=callbacks)
