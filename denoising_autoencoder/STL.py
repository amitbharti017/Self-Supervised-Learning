...
#### 2. Loading Dataset
train_dir = "../../datasets/STL/train/"
test_dir = "../../datasets/STL/validation/"
train_image_path = [train_dir+file for file in os.listdir(train_dir) if file.endswith(".png")]
test_image_path = [test_dir+file for file in os.listdir(test_dir) if file.endswith(".png")]
#### 5. Data Generator
...
model_STL=tf.keras.Model(inputs=[start],outputs=[outputs])
model_STL.compile(optimizer=Adam(learning_rate=0.0001),loss="mse", metrics=[tf.keras.metrics.MeanSquaredError()])
model_STL.summary()

checkpointer= tf.keras.callbacks.ModelCheckpoint("./models/ensemble/STL5.h5",verbose=1,save_best_only=True)

file_name="denoise_STL5"
...
history = model_STL.fit(train_generator,validation_data=val_generator,epochs=100,callbacks=callbacks)
